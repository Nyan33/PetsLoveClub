...
$returnTo = $_GET['return_to'] ?? ($_POST['return_to'] ?? '../news.php');
$cancelUrl = $existing ? ('../news.php?article=' . (int)$existing['id']) : '../news.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete' && $existing) {
        ...
        $pdo->prepare("DELETE FROM news WHERE id = :id")->execute([':id' => $existing['id']]);
        $back = $_POST['return_to'] ?? '../news.php';
    ...
    if ($values['content'] === '') $errors[] = 'Текст новости не может быть пустым.';
    if (!$existing && $imageData === '') $errors[] = 'Загрузите и обрежьте изображение.';

    if (!$errors) {
        $imagePath = $existing['image_url'] ?? null;
        if ($imageData !== '') {
            $saved = save_base64_image($imageData, 'news', 'n');
            if (!$saved) {
    ...
    if (!$errors) {
        if ($existing) {
            $upd = $pdo->prepare(
                "UPDATE news SET title=:t, slug=:s, published_at=:p, image_url=:i, description=:d, content=:c
                  WHERE id=:id"
            );
            $upd->execute([
                ':t' => $values['title'],
                ...
                ':i' => $imagePath,
                ':d' => $values['description'] !== '' ? $values['description'] : null,
                ':c' => $values['content'],
                ':id' => $existing['id'],
            ]);
            header('Location: ../news.php?article=' . (int)$existing['id']);
            exit;
        } else {
            $ins = $pdo->prepare(
                "INSERT INTO news (title, slug, published_at, image_url, description, content)
                 VALUES (:t, :s, :p, :i, :d, :c)"
            );
            $ins->execute([
                ':t' => $values['title'],
            ...
            header('Location: ../news.php?article=' . (int)$pdo->lastInsertId());
            exit;
        }
    }
}

$pageTitle = $existing ? 'Редактировать новость' : 'Новая новость';
?>
<!DOCTYPE html>
<html lang="ru">
<head>
...
<?php $active = 'news'; include __DIR__ . '/../includes/nav.php'; ?>
        ...
        <form method="post" id="content-form" class="bg-white rounded-3xl shadow-xl p-6 sm:p-8 border-2 border-gray-100 space-y-5">
            ...
            <div>
                ...
                <textarea name="description" rows="2" maxlength="500"
                          class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-rose-400 focus:outline-none transition-colors"><?= htmlspecialchars($values['description']) ?></textarea>
            </div>

            <div>
                <label class="block text-sm font-bold text-gray-700 mb-1.5">Текст новости (Markdown)</label>
